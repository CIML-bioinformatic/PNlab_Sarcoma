...
library( clusterProfiler)   # GO/KEGG enrichment & plots

#library( circlize)          # colorRamp2 for ComplexHeatmap
#library( ComplexHeatmap)    # Plot heatmaps

library( corrplot)          # Plot Pearson residuals for chisquare test
...
library( ggpubr)
library( ggrepel)
library( graphics)          # 'mosaicplot' (chisquare test)
#library( grid)
#library( gridExtra)
library( heatmaply)
...
#library( plyr)
#library( dplyr)
library( RFutils)           # GO_plotClusterAndGenes, GO_*
library( rlang)             # missing_arg
library( rmarkdown)
library( scales)            # hue_pal
library( Seurat)
library( stringr)           # str_trunc (for dotplot below)
library( webshot)




######## Dotplot for functional enrichments using plotly

# Create a dotplot from a data.frame with appropriate columns from a functional enrichment analysis (see clusterProfiler).
# It expects at least columns "GeneRatio", "p.adjust", "Description", "ID", "BgRatio", "geneID", "Count".
# As expected from clusterProfiler output, '*Ratio' values are fractions represented as strings (e.g. "4/159").
# Value for xColname can be any column name from input dataframe or "Gene ratio" (computed internally).
# Returns a plotly object, ready to be plotted.
dotPlotly = function( tableResultDF, xColName = "Gene ratio", sortColName = xColName, reverseSort = FALSE, figureTitle = "", topTerms = NULL, topTermsColumn = "p.adjust", reverseTop = FALSE)
{
  # Check arguments
  if(! (is.data.frame(tableResultDF) && all(c("GeneRatio", "p.adjust", "Description", "ID", "BgRatio", "geneID", "Count") %in% colnames(tableResultDF)))) stop("Argument 'tableResultDF' must be a data.frame with columns 'GeneRatio', 'p.adjust', 'Description', 'ID', 'BgRatio', 'geneID', 'Count'...")
  if(! (is.character(xColName) && (length(xColName) == 1) && (xColName %in% c(colnames(tableResultDF), "Gene ratio")))) stop("Argument 'xColName' must be a unique character string value refering to a valid column of 'tableResultDF' or 'Gene ratio'...")
  if(! (is.character(sortColName) && (length(sortColName) == 1) && (sortColName %in% c(colnames(tableResultDF), "Gene ratio")))) stop("Argument 'sortColName' must be a unique character string value refering to a valid column of 'tableResultDF'...")
  if(! (is.logical(reverseSort) && (length(reverseSort)==1) && is.finite(reverseSort))) stop("Argument 'reverseSort' must be a unique finite (not NA) logical value...")
  if(! (is.null(topTerms) || ( (length(topTerms) == 1) && is.numeric(topTerms) && is.finite(topTerms) && (topTerms>0)))) stop("Argument 'topTerms' must be a unique strictly positive finite number or NULL...")
  if(! (is.character(topTermsColumn) && (length(topTermsColumn) == 1) && (topTermsColumn %in% c(colnames(tableResultDF), "Gene ratio")))) stop("Argument 'topTermsColumn' must be a unique character string value refering to a valid column of 'tableResultDF'...")
  if(! (is.logical(reverseTop) && (length(reverseTop)==1) && is.finite(reverseTop))) stop("Argument 'reverseTop' must be a unique finite (not NA) logical value...")
  
  # Compute actual gene ratio value from fractional representation in data.frame
  tableResultDF[["Gene ratio"]] = sapply( parse( text = tableResultDF[["GeneRatio"]]), eval)
  
  # Select 'top terms' based on selected column
  if(!is.null(topTerms))
  {
    tableResultDF = tableResultDF[order( tableResultDF[[topTermsColumn]], decreasing = reverseTop),] # Sort it
    tableResultDF = head(tableResultDF, topTerms) # Select first occurences
  }
  
  # Sort dataframe according to selected column for x axis
  tableResultDF = tableResultDF[order( tableResultDF[[sortColName]], decreasing = reverseSort),]
  
  if(!nrow(tableResultDF))
  {
    warning("Empty table, skipping dotplot (return NULL)...")
    return(NULL);
  }
  
  plot_ly( tableResultDF,
           x = ~get(xColName),
           y = ~Description) %>% 
    add_markers( size = ~Count, 
                 marker  = list( sizeref = 0.1,
                                 line = list(width = 0)),
                 customdata = tableResultDF[["Count"]],
                 color = ~-log10(p.adjust),
                 colors = c("darkblue", "red"),
                 hoverinfo = 'text',
                 text = paste0( "<b>", str_trunc(tableResultDF[["Description"]], 30), "</b>\n\n",
                                "ID: ", tableResultDF[["ID"]], "\n",
                                "GeneRatio: ", tableResultDF[["GeneRatio"]], "\n",
                                "BgRatio: ", tableResultDF[["BgRatio"]], "\n",
                                "gene(s): ", tableResultDF[["geneID"]], "\n",
                                "Adj.Pval: ", format.pval(tableResultDF[["p.adjust"]]), "\n")) %>%
    layout( title = figureTitle, 
            yaxis = list( categoryarray = ~Description,
                          title = ""),
            xaxis = list( title = xColName)) %>% # Force the ordering of categorical variable on y-axis (reordering initial factor raises error: 'range' not meaningful for factors)
...
# NOTE: This version assumes a column named "jitterCoords" previously created in 
# plotData so it allows to use a "SharedData" object that we cannot modify here.
...
violinFeatureByCluster = function(currentFeature, seuratObject, slot = "counts", clustersColor = NULL, yLabel = "Counts", addStats = TRUE, addTitle = TRUE, trimTitle = FALSE) 
{
  ...
  if(addStats)
  {
    ...
    ggFigure = ggFigure + geom_text( data = data.frame( Stat = statsZero, Cluster = names( statsZero)), 
      ...
      geom_text( data = data.frame( text = "Pos.\nNull"), 
                 aes( label = text), 
                 x = -0.4, y = max( featureValues)*1.1, hjust = 0, size = 3) +
  ...
  if(addTitle)
  {
    ...
    # Create the title layer to be added (copy 'FeaturePlot' title style from cowplot, but a bit larger fo higher resolution figures)
...
######## Create a violinplot showing feature (gene or metadata) values for each cluster (+ number of 'zero' and 'not zero' cells)
# Adding split by a custom metadata factor (and return the plot instead of rendering it)
violinFeatureByCluster_metadataColSplit = function(currentFeature, seuratObject, slot = "counts", clustersColor = NULL, yLabel = "Counts", addStats = TRUE, addTitle = TRUE, trimTitle = FALSE, metadataColSplit = "factorHTO") 
{
  if(length( currentFeature)>1) warning( "Several features in 'currentFeature' argument, corresponding values will be averaged...");
  ...
  # Plot expression values of current annotation for each cluster as violin + jitter
  ggFigure = ggplot( cbind( data.frame( Counts = featureValues, Cluster = Idents( seuratObject)), seuratObject[[]]), 
                     aes( x = Cluster, y = Counts, group = )) +
    geom_jitter( width = 0.2, height = 0.3, size = 0.4, colour = "#44444444") +
    geom_violin( aes( col = Cluster, fill = Cluster), scale = "width", alpha = 0.4, draw_quantiles = 0.5) +
    theme_minimal() +
    ylab( label = yLabel) +
    ...
    theme( legend.position = "None",
           axis.text.x = element_text( angle = 45,
                                       hjust = 1)) +
    facet_grid(reformulate(".", metadataColSplit));
  ...
  {
    # Get the number of cells in which score is zero (or not) for this annotation
    splitStatList = list(Idents( seuratObject), seuratObject[[metadataColSplit, drop = TRUE]])
    names(splitStatList) = c("Cluster", metadataColSplit)
    statsZero = tapply( featureValues, 
                        splitStatList,
                        function(x) { paste( table( factor( as.logical( x), levels = c( TRUE, FALSE))), collapse="\n") });
    
    statsZeroDF = reshape2::melt(statsZero)
    
    # Create the text layer with stats to be added
    ggFigure = ggFigure + geom_text( data = statsZeroDF, 
                                     aes( x = Cluster, y = max( featureValues) *1.0, label = value), 
                                     size = 3)
  } 
