...
# In this version EXTERNAL_CLUSTERING_PATH is a table of file path instead of a single one
if(all(file.exists( EXTERNAL_CLUSTERING_PATH)))
{
  externalClusters = do.call( rbind, lapply(EXTERNAL_CLUSTERING_PATH, read.csv , sep = "\t", row.names = 1, stringsAsFactors = TRUE));
...
} else stop("At least one file specified for 'EXTERNAL_CLUSTERING_PATH' could not be found")
...
# PREPARE DATA, ONLY FOR SEARCHING MARKER GENES 
###############################################

# Recompute normalization, variable genes, and PCA in case subsetting was done
# manually (see note in analysisParams) and corresponding slots does not match
# data in Seurat object.
# Does not give typical output tables and graphs for these steps.
