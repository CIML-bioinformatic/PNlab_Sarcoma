...
######## Create a subchunk to specify different options in  a subsection

subchunkify <- function(g, options = "fig.dim=c(7,7), out.width='100%'") {
  g_deparsed <- paste0(deparse(
    function() {g}
  ), collapse = '')
  
  sub_chunk <- paste0("
```{r sub_chunk_", digest(g, algo="crc32"), ", ", options, ", echo=FALSE}",
                   "\n(", 
                   g_deparsed
                   , ")()",
                   "\n```
