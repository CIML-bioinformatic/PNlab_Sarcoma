...
######## Create a violinplot showing feature (gene or metadata) values for each cluster (+ number of 'zero' and 'not zero' cells)
violinFeatureByMetadata = function(currentFeature, seuratObject, slot = "counts", metadataName = "factorHTO", metadataColor = NULL, yLabel = "Counts", addStats = TRUE, addTitle = TRUE, trimTitle = FALSE, byCluster = TRUE) 
{
  if(length( currentFeature)>1) warning( "Several features in 'currentFeature' argument, corresponding values will be averaged...");
  ...
  # Plot expression values of current annotation for each cluster as violin + jitter
  ggFigure = ggplot( data.frame( Counts = featureValues, Cluster = Idents( seuratObject), meta = seuratObject[[metadataName, drop = TRUE]]), 
                     aes( x = meta, y = Counts)) +
    geom_jitter( width = 0.2, height = 0.3, size = 0.4, colour = "#44444444") +
    geom_violin( aes( col = meta, fill = meta), scale = "width", alpha = 0.4, draw_quantiles = 0.5) +
    theme_minimal() +
    ylab( label = yLabel) +
    scale_x_discrete( labels=function(x){ sub( "\\s", "\n", x) }) +
    theme( legend.position = "None",
  ...
  if(byCluster)
  {
    ggFigure = ggFigure + facet_wrap(vars(Cluster), nrow = 1)
  }
  # Change ggplot default clusters colors ('clustersColor' should be a named vector of colors)
  if(!is.null(metadataColor))
  {
    ggFigure = ggFigure + 
      scale_color_manual( values = metadataColor) +
      scale_fill_manual( values = metadataColor);
  }
