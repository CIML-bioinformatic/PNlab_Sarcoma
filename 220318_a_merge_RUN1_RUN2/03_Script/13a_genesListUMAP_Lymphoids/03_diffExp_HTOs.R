    ...
    for(currentIdentity in names(topDiffExp_nestedList[[currentComparisonName]]))
    {
      ...
      for(currentCat in c("Pos", "Neg"))
      {
        ...
        if(nrow(currentSelection) > 0)
        {
          ...
          DEdf_noFilter[selectedLabels, "label"] = selectedLabels;
        }
      }
      
      # Prepare graphic adjustment values
      categoryColors = c("darkred", "darkblue", "black")
      ...
      if(any( DEdf_noFilter[["label"]] != "" ))
      {
        volcano = volcano +
          geom_text_repel( data = DEdf_noFilter[ DEdf_noFilter[["category"]] %in% c("Pos", "Neg"), ], # Select all DEGs to repel on points ("" label)
                           ...
                           show.legend = FALSE,
                           segment.size = 0.15)
          # geom_text_repel( data = DEdf_noFilter[ DEdf_noFilter[["category"]] == "Pos", ], # Select all 'Pos' DEGs to repel on points ("" label)
          #                  aes(label = label),
          #                  size = 4,
          #                  xlim = c(0.4, max(DEdf_noFilter[["avg_log2FC"]])),
          #                  show.legend = FALSE,
          #                  segment.size = 0.15) +
          # geom_text_repel( data = DEdf_noFilter[ DEdf_noFilter[["category"]] == "Neg", ], # Select all 'Neg' DEGs to repel on points ("" label)
          #                  aes(label = label),
          #                  size = 4,
          #                  xlim = c(min(DEdf_noFilter[["avg_log2FC"]]), -0.4),
          #                  show.legend = FALSE,
          #                  segment.size = 0.15)
      }
      ...
      categoryColors = c("grey", "darkorange", "darkgreen")
      names(categoryColors) = c("Unselected", names(cellsList))
      ...
      umapCellsDE = DimPlot( sc10x, 
                      ...
                      scale_color_manual( labels = categoryNames, values = categoryColors) +
                      theme_classic()
        ...
        for( currentIdentity in names(enrichmentList[[currentComparisonName]][[currentCategoryName]]))
        {
          ...
          if(!is.null( dotplotFigure)) # Function returns NULL on empty enrichments
          {
            ...
            if(currentCategoryName == "K-Pathways")
            {
              ...
              # Convert to entrez IDs using precomputed table
