...
## @knitr heatmap_compute_expression_summary_matrices
...
# Multiplier for pixel size (width/height) when exporting png files
pngSizeMultiplier = 1.5
                        ...
                        headerLevel = 6) # Header level to start from for rmarkdown titles (typically one more than current title level, which defines a tabset itself)
{
  ...
  if(!is.null(columnsCategories))
  {
    columnsAnnot = HeatmapAnnotation( df = columnsCategories,
                                      col = columnsCategoriesColor,
                                      annotation_legend_param = lapply(columnsCategoriesColor, function(ignored){list( labels_gp = gpar(fontsize = 15),
                                                                                                                       title_gp = gpar(fontsize = 15), # Random colors if NULL 
                                                                                                                       grid_width = unit(0.4, "inch"),
                                                                                                                       grid_height = unit(0.4, "inch"))}))  
  }
  ...
  for(scaleArg in (if(nrow(avgExpressionMatrix) > 1) scaleArgs else "none")) # No need to rescale if single row of data
  {
    ...
    hm = Heatmap( if(scaleArg == "rows") t(scale(t(avgExpressionMatrix))) else avgExpressionMatrix, 
                  ...
                  heatmap_legend_param = list( labels_gp = gpar(fontsize = 15),
                                               title_gp = gpar(fontsize = 15),
                                               grid_width = unit(0.7, "inch"),
                                               legend_height = unit(3, "inch"))) 
    ...
    if(!is.null(basePath))
    {
      ...
      } else # If it's the first run, create the figure as external file
      {
        ...
        png( filenamePNG, 
             width = (width)*pngSizeMultiplier, 
             height = (height)*pngSizeMultiplier)
        draw( hm)
        dev.off()
    ...
    for(scaleArg in scaleArgs)
    {
      ...
      if(!is.null(basePath))
      {
        ...
        } else # If it's the first run, create the figure as external file
        {
          ...
          # Don't include them in report, will be done in secind run.
          png( filenamePNG, 
               width = (width+50)*pngSizeMultiplier, # Add 50 pixels to accomodate for dendrogram 
               height = (height)*pngSizeMultiplier)
